...
from models.deeplabv3.deeplabv3_model import DeepLabV3, DeepLabV3Plus
from models.DBTSNet.DBTSNet_attention import DBTSNet
warnings.filterwarnings("ignore")
# Add the parser initialization line here
parser = argparse.ArgumentParser(description='PyTorch CIFAR10 Training')
parser.add_argument('--imgs_train_list', type=str,default=r'./DBTSnet/preprocessed_data/csv/train.csv', help='Path to training data list')
parser.add_argument('--imgs_val_list', type=str,default=r'./DBTSnet/preprocessed_data/csv/val.csv', help='Path to validation data list')
...
parser.add_argument('--checkpoint', type=str, default='checkpoint/', help='Base directory for saving checkpoints')
parser.add_argument('--name', type=str, default='resnext101_32x4d', help='Model name identifier')
parser.add_argument('--save_name', type=str, default= 'T2_Deeplabv3_resnext50_32x4d', help='Custom save name for the model')
parser.add_argument('--custom_save_dir', type=str, default='', help='Optional custom directory to save all model files, overrides checkpoint and save_name if provided')
parser.add_argument('--devicenum', default='0', type=str, )
parser.add_argument('--model_type', type=str, default='DBTS-Net', choices=['DBTS-Net'], help='Model type to use: DBTS-Net ')
...
if args.custom_save_dir:
    # 如果提供了自定义保存目录，直接使用
    model_savedir = args.custom_save_dir.rstrip('/') + '/'
else:
    # 否则使用默认逻辑
    model_savedir = args.checkpoint + args.save_name.replace("resnext50_32x4d", args.name) + '/'
    
save_name = model_savedir + 'ckpt'
print(f"模型将保存到目录: {model_savedir}")
...
train_imgs,train_masks = train_csv['image_name'],train_csv['mask_name']
val_imgs,val_masks = val_csv['image_name'],val_csv['mask_name']

# Only use Linux paths since we're running on a remote Linux server
# Remove Windows paths entirely
train_imgs = [''.join([r'./preprocessed_data/train/image','/',i]) for i in train_imgs]
train_masks = [''.join([r'./preprocessed_data/train/label','/',i]) for i in train_masks]
val_imgs = [''.join([r'./preprocessed_data/val/image','/',i]) for i in val_imgs]
val_masks = [''.join([r'./preprocessed_data/val/label','/',i]) for i in val_masks]
...
def main():
    ...
    # 使用DBTS-Net模型
    model = DBTSNet(encoder_name=args.name, encoder_weights=None, in_channels=3, classes=2, activation=None).to(device)
