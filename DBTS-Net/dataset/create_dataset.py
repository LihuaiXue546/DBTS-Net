...
class Mydataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # 添加检查确保图像不为None
        if img is None:
            # 如果图像为None，创建一个默认的空白图像
            print(f"警告: 索引 {index} 处的图像加载失败，创建空白图像")
            img = np.zeros((480, 480), dtype=np.float32)
        else:
            # 确保图像是numpy数组
            if isinstance(img, str):
                # 如果是文件路径，尝试读取图像
                print(f"警告: 索引 {index} 处是文件路径而非图像数据: {img}")
                try:
                    img = cv2.imread(img, cv2.IMREAD_GRAYSCALE)
                    if img is None:
                        img = np.zeros((480, 480), dtype=np.float32)
                        print(f"警告: 无法读取图像文件: {img}")
                except Exception as e:
                    img = np.zeros((480, 480), dtype=np.float32)
                    print(f"警告: 读取图像时出错: {e}")
